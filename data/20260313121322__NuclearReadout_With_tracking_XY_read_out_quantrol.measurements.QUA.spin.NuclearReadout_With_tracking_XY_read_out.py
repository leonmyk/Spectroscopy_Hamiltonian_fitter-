...
class NuclearReadout_With_tracking_XY_read_out(QMSpinMeasurement):
    ...
    def __init__(self,
        ...
        self.n_ro_nuclear = n_ro_nuclear
    ...
    def nuclear_spin_RO_XY(
        ...
        n_ro_set_X = declare(int)
        n_ro_set_Y = declare(int)
        delta_Y = declare(int)
        delta_X1 = declare(int)
        delta_X2 = declare(int)
        freq_set = declare(int)
        click_tmp = declare(int)
        readout_freqs_Y = [(readout_freqs[0]+readout_freqs[2])//2,(readout_freqs[1]+readout_freqs[3])//2]
        readout_freqs_X = readout_freqs
        
    
        with for_(n_ro_set_Y, 0, n_ro_set_Y < n_ro_nuclear_Y, n_ro_set_Y + 1):
            

            # Runs nuclear_readout_block function once for each frequency
            assign(freq_set, readout_freqs_Y[0])
            self.nuclear_readout_block_large(click_tmp, freq_set + delta_freq)
            save(click_tmp, ystream)
            # assign(delta_Y,delta_Y+click_tmp)
            
            # Runs nuclear_readout_block function once for each frequency
            assign(freq_set, readout_freqs_Y[1])
            self.nuclear_readout_block_large(click_tmp, freq_set + delta_freq)
            save(click_tmp, ystream)
            # assign(delta_Y,delta_Y-click_tmp)
        
        with for_(n_ro_set_X, 0, n_ro_set_X < n_ro_nuclear_X, n_ro_set_X + 1):
            
            # Runs nuclear_readout_block function once for each frequency
            assign(freq_set, readout_freqs_X[0])
            self.nuclear_readout_block(click_tmp, freq_set + delta_freq)
            save(click_tmp, xstream)
            assign(delta_X1,click_tmp)
            
            # Runs nuclear_readout_block function once for each frequency
            assign(freq_set, readout_freqs_X[1])
            self.nuclear_readout_block(click_tmp, freq_set + delta_freq)
            save(click_tmp, xstream)
            # assign(delta_X,delta_X-click_tmp)
        
            # Runs nuclear_readout_block function once for each frequency
            assign(freq_set, readout_freqs_X[2])
            self.nuclear_readout_block(click_tmp, freq_set + delta_freq)
            save(click_tmp, xstream)
            # assign(delta_Y,delta_Y+click_tmp)
            
            # Runs nuclear_readout_block function once for each frequency
            assign(freq_set, readout_freqs_X[3])
            self.nuclear_readout_block(click_tmp, freq_set + delta_freq)
            save(click_tmp, xstream)
            # assign(delta_X,delta_X-click_tmp)
        ...
        with program() as prog:
            ...
            with for_(k, 0, k < self.n_iterations, k + 1):
                ...
                with for_(state_to_prepare, 0, state_to_prepare < len(self.states_to_prepare), state_to_prepare + 1):
                    ...
                    self.nuclear_spin_RO_XY(x_stream,y_stream, self.nro_delta_y,self.nro_delta_x, self.ro_freqs, delta_freq, accumulate=False)
            ...
            with stream_processing():
                ...
                x_stream.buffer(4).buffer(self.nro_delta_x ).buffer(len(self.states_to_prepare)).save_all('clicksX')
                y_stream.buffer(2).buffer(self.nro_delta_y).buffer(len(self.states_to_prepare)).save_all('clicksY')
    ...
    def analysis(self, data):
        ...
        clicksY  = np.array([sublist[0] for sublist in data.clicksY.fetch_all()])
    ...
    def plot(self, signal: dict, fit: dict):

        figs = []
        
        
        clicksX = signal['clicksX']
        clicksY = signal['clicksY']
        avg = clicksX.shape[0]
        nb_prep_states = clicksX.shape[1]
        xnro = clicksX.shape[2]
        
        print('clciks shape is ',clicksX.shape)  #clicks shape is  (43, 4, 50, 4)
        ...
        for prepped_state in self.states_to_prepare:
            ...
            dx = clicksX[:,prepped_state,:,0].sum(1) + clicksX[:,prepped_state,:,1].sum(1) - clicksX[:,prepped_state,:,2].sum(1) - clicksX[:,prepped_state,:,3].sum(1)
            dy =  clicksY[:,prepped_state,:,1].sum(1) - clicksY[:,prepped_state,:,0].sum(1)
